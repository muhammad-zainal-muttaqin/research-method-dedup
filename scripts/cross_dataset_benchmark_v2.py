"""
Cross-Dataset Benchmark: v9 vs v10 (PER-FILE VERSION)
Evaluasi generalisasi algoritma pada 3 dataset:
1. json/ (22 Apr 2026) - 228 files
2. json_28 April 2026/ - 478 files  
3. json_30 April 2026/ - 727 files (largest)

Setiap JSON file adalah satu tree instance yang dievaluasi.
...
def load_dataset(json_dir: str):
    """Load dataset - return list of files."""
    ...
    print(f"  Loading {len(files)} JSON files from {json_dir}...")
    return files


def parse_json_file(json_file: Path) -> tuple:
    """Parse single JSON file -> detections + GT counts."""
    ...
    return detections, gt_counts, data.get("tree_id", json_file.stem)


def evaluate_file(detections: list, gt_counts: dict, method_fn, method_name: str, params=None) -> dict:
    """Evaluate single file dengan method."""
    if params:
...
def evaluate_dataset(files: list, dataset_name: str, v9_params: dict) -> pd.DataFrame:
    """Evaluate seluruh dataset dengan v9 dan v10."""
    results = []
    
    print(f"\nEvaluating {dataset_name} ({len(files)} files)...")
    
    for json_file in files:
        try:
            detections, gt_counts, tree_id = parse_json_file(json_file)
            if not detections or sum(gt_counts.values()) == 0:
                continue
            
            # v9
            r9 = evaluate_file(detections, gt_counts, v9_predict, "v9", v9_params)
            r9["file"] = json_file.name
            r9["tree_id"] = tree_id
            r9["dataset"] = dataset_name
            results.append(r9)
            
            # v10
            r10 = evaluate_file(detections, gt_counts, v10_predict, "v10", None)
            r10["file"] = json_file.name
            r10["tree_id"] = tree_id
...
def main():
    print("=" * 60)
    print("CROSS-DATASET BENCHMARK: v9 vs v10 (PER-FILE)")
    print("=" * 60)
    ...
    for name, path in datasets.items():
        ...
        df = evaluate_dataset(files, name, v9_params)
    ...
    combined.to_csv(report_dir / "detailed_results_v2.csv", index=False)
    
    # Create comparison table
    print(f"\n\n{'='*60}")
    print("FINAL COMPARISON TABLE (PER-FILE)")
    print(f"{'='*60}")
    print(f"\n{'Dataset':<15} {'Files':<8} {'v9 Acc':<12} {'v10 Acc':<12} {'v9 MAE':<12} {'v10 MAE':<12} {'Delta Acc':<10}")
    print(f"{'-'*85}")
    
    comparison_rows = []
    for name in datasets.keys():
        if name in all_summaries:
            n_files = all_summaries[name]["v9"]["n_files"]
            ...
            print(f"{name:<15} {n_files:<8} {v9_acc:.2f}%       {v10_acc:.2f}%       {v9_mae:.4f}       {v10_mae:.4f}       {delta:+.2f}%")
    ...
    comparison_df.to_csv(report_dir / "comparison_summary_v2.csv", index=False)
    
    # Save detailed summary
    with open(report_dir / "summary_report_v2.txt", 'w') as f:
        f.write("CROSS-DATASET BENCHMARK: v9 vs v10 (PER-FILE)\n")
        f.write("="*60 + "\n\n")
    ...
    print(f"  - comparison_summary_v2.csv")
    print(f"  - summary_report_v2.txt")
