"""
Regenerate README sample visualizations at dataset root:
...
JSON_DIR = BASE / "json"
IMG_DIR = BASE / "images"
OUT_DIR = BASE  # write at dataset root
...
def compose(tree_name, data, pw, ph, cols, out_path):
    ...
    n_pairs = sum(1 for b in bunches if b.get("appearance_count", 1) > 1)
    n_sides = len(panels)
    summary = (f"{tree_name}   |   {n_sides} views   |   {n_unique} unique bunches   |   "
